...
// ─────────────────────────────────────────────────────────────────────────────
// Constants (identical to new-invoice page)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// calcLine helper (identical to new-invoice page)
// ─────────────────────────────────────────────────────────────────────────────
...
export default function EditInvoicePage() {
  ...
  // Customer modal — identical to new-invoice page
  ...
  // Tax modal — identical to new-invoice page
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setItems(allItems);
        setGlobalTaxes(Array.isArray(allTaxes) ? allTaxes : []);
        ...
        // Split description on \n so the catalogue <select> matches item.name
        setLineItems(inv.items.map((l: any) => {
        ...
        const found = allCustomers.find((c: Customer) => c._id === inv.customer_id) ?? null;
  ...
  // Re-sync selectedCustomer only when user changes dropdown (not on initial load)
  useEffect(() => {
  ...
  function addTaxToLine(lineIndex: number, tax: Tax) {
    setLineItems(prev => prev.map((l, i) => {
      ...
      if (l.taxes.find((t: any) => t.tax_id === tax.tax_id)) return l;
  ...
  function removeTaxFromLine(lineIndex: number, taxIndex: number) {
    setLineItems(prev => prev.map((l, i) => {
      if (i !== lineIndex) return l;
      return { ...l, taxes: l.taxes.filter((_: any, ti: number) => ti !== taxIndex) };
    }));
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (dueDate && dueDate < today) errs.dueDate = 'Due date must be today or a future date';
    lineItems.forEach((l, i) => { if (!l.description.trim()) errs[`desc_${i}`] = 'Required'; });
    setErrors(errs);
    if (Object.keys(errs).length > 0) { toast.error('Please fix validation errors'); return; }
    setSaving(true);
    try {
      const payload = {
        ...
        items: lineItems.map(l => ({
          ...
          taxes: l.taxes.map((t: any) => ({ tax_id: t.tax_id, name: t.name, percent: t.percent })),
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="bg-white border-b border-slate-200 px-8 py-4 flex items-center justify-between sticky top-0 z-10">
          <div className="flex items-center gap-4">
            <button type="button" onClick={() => router.push(`/invoices/${id}`)} className="text-slate-400 hover:text-slate-600 cursor-pointer">
          ...
          <div className="flex items-center gap-3">
            <button type="button" onClick={() => router.push(`/invoices/${id}`)} className="px-4 py-2 text-sm rounded-md border border-slate-200 hover:bg-slate-50 cursor-pointer">
              Cancel
            </button>
            <button type="submit" disabled={saving} className="flex items-center gap-2 bg-blue-600 text-white px-6 py-2 rounded-md text-sm font-bold hover:bg-blue-700 disabled:opacity-50 cursor-pointer">
        ...
        <div className="max-w-6xl mx-auto px-8 py-8 space-y-8">
            ...
            <div className="col-span-6 bg-white rounded-xl border border-slate-200 p-6">
              ...
              {selectedCustomer ? (
                <div className="relative">
                  ...
                  <button type="button" onClick={() => { setSelectedCustomer(null); setCustomerId(''); }} className="absolute top-2 right-2 text-slate-400 hover:text-slate-600 cursor-pointer">
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ) : (
                <div className="space-y-3">
                  <select value={customerId} onChange={e => setCustomerId(e.target.value)} className={inputClass}>
                    ...
                    {customers.map(c => (
                      <option key={c._id} value={c._id}>{c.customer_name}{c.company_name ? ` — ${c.company_name}` : ''}</option>
                    ))}
                  </select>
            ...
            <div className="col-span-6 bg-white rounded-xl border border-slate-200 p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="date" value={issueDate} readOnly className={`${inputClass} mt-1 bg-slate-50 text-slate-500 cursor-not-allowed`} />
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-600 uppercase">Due Date</label>
                  <input type="date" value={dueDate} min={today} onChange={e => setDueDate(e.target.value)} className={`${inputClass} mt-1`} />
                ...
                <div>
                  ...
                  <input value={paymentTerms} onChange={e => setPaymentTerms(e.target.value)} placeholder="e.g. Net 30" className={`${inputClass} mt-1`} />
                </div>
              </div>
            </div>
          </div>
                    ...
                    <div key={i} className="group border-b border-slate-100 relative">
                      ...
                      <div className="grid grid-cols-12 gap-4 px-8 pt-4 pb-2 items-center">
                        ...
                        <div className="col-span-2">
                          <input type="number" min="1" step="1" value={line.quantity} onChange={e => updateLine(i, 'quantity', e.target.value)} className={inputClass} />
                        </div>
                        <div className="col-span-2">
                      ...
                      <div className="grid grid-cols-12 gap-4 px-8 pb-3 items-start">
                        ...
                        <div className="col-span-2 space-y-1.5">
                          {(line.taxSlots ?? []).map((_: any, slotIdx: number) => (
                            <select
                              key={slotIdx}
              ...
              {discountAdded ? (
                <div className="flex justify-between items-center text-sm text-slate-700">
                  <div className="flex items-center gap-2">
                    ...
                    <button type="button" onClick={() => { setDiscountAdded(false); setDiscountPercent('0'); setDiscountAmount('0'); }} className="text-slate-300 hover:text-red-400 cursor-pointer">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6 grid grid-cols-2 gap-6">
            <div>
              ...
              <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3} placeholder="Any notes for the customer…"
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm">
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              ...
              <button onClick={() => setShowTaxModal(false)} className="text-slate-400 hover:text-slate-600 cursor-pointer"><X className="w-5 h-5" /></button>
            </div>
            <form onSubmit={handleCreateTax} className="p-6 space-y-4">
               ...
               <div className="flex justify-end gap-3 pt-4">
                 <button type="button" onClick={() => setShowTaxModal(false)} className="px-4 py-2 font-medium text-slate-600 hover:bg-slate-50 rounded cursor-pointer">Cancel</button>
                 <button type="submit" disabled={savingTax} className="px-6 py-2 bg-blue-600 text-white font-bold rounded hover:bg-blue-700 flex items-center gap-2 cursor-pointer disabled:opacity-60">
